...
import { Dumbbell, ShieldCheck, CheckCircle2, MapPin, Phone } from 'lucide-react';
import { FACILITIES } from '@/data/clubData';
import { SITE_CONFIG, constructMetadata, getJsonLdSchemas } from '@/lib/seo';

export const metadata = constructMetadata({
  title: "Boxing Facilities in Kochi & Ernakulam | CUSAT Boxing Gym",
  description: "Tour our state-of-the-art boxing gym facilities in Kalamassery, Ernakulam featuring competition ring, leather heavy bags, reflex bags, and athletic turf.",
  canonical: "/facilities",
});

export default function FacilitiesPage() {
  const jsonLdSchemas = getJsonLdSchemas("/facilities");
      ...
      <main className="min-h-screen pt-32 pb-24 bg-[#070707] text-white">
        ...
        <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#141414] border border-[#C89B3C]/40 text-[#C89B3C] text-xs font-semibold uppercase tracking-widest mb-6">
            ...
            <span>GYM INFRASTRUCTURE & EQUIPMENT • ERNAKULAM</span>
          </div>

          <h1 className="font-heading text-6xl sm:text-7xl lg:text-8xl tracking-tight text-white uppercase max-w-4xl mx-auto">
            BOXING FACILITIES & <span className="gold-gradient-text">ARENA IN KOCHI & ERNAKULAM</span>
          </h1>

          <p className="mt-6 text-neutral-300 text-lg sm:text-xl max-w-2xl mx-auto leading-relaxed">
            State-of-the-art combat sports facilities inside CUSAT Sports Complex, South Kalamassery, Kochi. Fully equipped for technical sparring, power bag training, strength conditioning, and youth cadet safety across Ernakulam district.
          </p>
        </section>
          ...
          {FACILITIES.map((facility, index) => (
            ...
            >
              ...
              <div className="lg:col-span-6 h-80 sm:h-96 rounded-2xl overflow-hidden relative group border border-white/10">
                <Image
                  src={facility.image}
                  alt={`${facility.name} at CUSAT Boxing Club in Kalamassery, Ernakulam`}
                  fill
        ...
        <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto mt-20">
          <div className="p-8 sm:p-12 rounded-3xl bg-gradient-to-r from-[#141414] via-[#1c1914] to-[#141414] border border-[#C89B3C]/50 text-center space-y-6">
            ...
            <p className="text-neutral-300 max-w-2xl mx-auto text-base leading-relaxed">
              Visitors, parents of youth cadets, and interested athletes from across Ernakulam and Kochi are welcome to visit our front desk during operating hours to tour our arena in South Kalamassery.
            </p>
